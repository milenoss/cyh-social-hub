/*
# Fixed Challenge Check-in Functions

1. New Functions
   - `check_in_challenge` - Properly qualified column references
   - `get_challenge_history` - Retrieves check-in history
...
3. Changes
   ...
   - Added proper JSON return types
*/
  ...
  SET 
    ...
    check_in_notes = CASE 
      WHEN note_text IS NOT NULL THEN
        COALESCE(challenge_participants.check_in_notes, '[]'::jsonb) || 
        json_build_object('date', today_date, 'note', note_text)::jsonb
      ELSE
        challenge_participants.check_in_notes
    END,
    completed_at = CASE 
