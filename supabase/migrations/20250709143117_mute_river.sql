/*
  ...
  1. New Functions
    - `get_friends()` - Get all friends for the current user
    - `get_friend_requests(status)` - Get friend requests by status
    - `send_friend_request(recipient_id, message)` - Send a friend request
    ...
    - `search_users(search_query, limit, offset)` - Search for users
    - `get_friend_suggestions(limit)` - Get friend suggestions
    - `check_friendship_status(target_user_id)` - Check friendship status
  
  2. Fixes
    - Drops existing conflicting functions
    - Fixes ambiguous column references
    - Fixes FOUND usage in remove_friend function
*/
...
-- Create remove_friend function (fixed to use ROW_COUNT instead of FOUND)
