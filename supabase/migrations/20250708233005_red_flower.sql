/*
# Fix Friendship Policies

1. New Tables
  - Adds friendship-related tables if they don't exist yet
  - Adds policies with IF NOT EXISTS checks to prevent errors

2. Security
  - Enables RLS on friendship tables
  - Creates policies for managing friend relationships
  - Adds functions for friend management

3. Changes
  - Uses IF NOT EXISTS for all policy creation statements
...
-- Create policies for friendships with IF NOT EXISTS checks
DO $$
BEGIN
  IF NOT EXISTS (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'friendships' AND policyname = 'Users can delete their own friendships'
  ) THEN
...
-- Create policies for friend requests with IF NOT EXISTS checks
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'friend_requests' AND policyname = 'Users can view friend requests they''ve sent or received'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'friend_requests' AND policyname = 'Users can send friend requests'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'friend_requests' AND policyname = 'Users can update friend requests they''ve received'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'friend_requests' AND policyname = 'Users can delete friend requests they''ve sent or received'
  ) THEN
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_friend_requests_updated_at'
  ) THEN
...
-- Create or replace function to send friend request
...
BEGIN
  ...
  -- Log the event if the function exists
  BEGIN
