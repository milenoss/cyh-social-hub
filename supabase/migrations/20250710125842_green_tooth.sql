...
1. New Tables
  - `challenge_comments` - For storing comments on challenges
  - `comment_likes` - For tracking likes on comments

2. Changes
  - Add columns to `challenge_participants`: check_in_notes, check_in_streak, last_check_in
  - Create new version of check_in_challenge function to fix conflicts
  - Add functions for comments, likes, and participant tracking

3. Security
  - Enable RLS on new tables
  - Add appropriate policies for comments and likes
*/
...
-- Create a fixed version of the check-in function with a different name to avoid conflicts
CREATE OR REPLACE FUNCTION public.check_in_challenge_v2(
...
-- Update the RPC function to use the new check-in function
...
BEGIN
  ...
  RETURN check_in_challenge_v2(challenge_id_param, note_text);
END;
$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'update_challenge_comments_updated_at'
  ) THEN
...
-- Drop existing function if it exists to avoid return type conflict
DROP FUNCTION IF EXISTS get_challenge_comments(uuid, integer, integer);
...
-- Drop existing function if it exists to avoid return type conflict
DROP FUNCTION IF EXISTS add_challenge_comment(uuid, text, uuid);
...
-- Create a function to pin/unpin a comment
...
BEGIN
  ...
  -- Get comment details
  SELECT challenge_id, is_pinned INTO challenge_id_val, is_pinned_val
  FROM challenge_comments
  WHERE id = comment_id_param;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM challenges 
    WHERE id = challenge_id_val AND created_by = auth.uid()
  ) THEN
    RETURN jsonb_build_object(
      'success', false,
      'message', 'Only the challenge creator can pin comments'
    );
  END IF;
  
  -- Toggle pin status
  UPDATE challenge_comments
  SET is_pinned = NOT is_pinned_val
  WHERE id = comment_id_param
  RETURNING is_pinned INTO is_pinned_val;
  
  RETURN jsonb_build_object(
    'success', true,
    'message', CASE WHEN is_pinned_val THEN 'Comment pinned' ELSE 'Comment unpinned' END,
...
-- Create a function to get challenge history
CREATE OR REPLACE FUNCTION get_challenge_history(challenge_id_param UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- Get history entries from check-in notes
  history_entries := participation_record.check_in_notes;
  
  -- Return history
  RETURN jsonb_build_object(
    'success', true,
    'participation', to_jsonb(participation_record),
    'history', COALESCE(history_entries, '[]'::jsonb)
  );
END;
$$;