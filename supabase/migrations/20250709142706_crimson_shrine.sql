...
1. Changes
   - Drop existing functions with proper parameter signatures
   - Recreate get_friends() function
   - Recreate get_friend_requests() function
   - Recreate request_account_deletion() function
   - Recreate cancel_account_deletion() function

2. Security
   - All functions use SECURITY DEFINER to run with the privileges of the function creator
   - Functions properly check user authentication
*/

-- Drop existing functions first with their proper parameter signatures
DROP FUNCTION IF EXISTS get_friends();
DROP FUNCTION IF EXISTS get_friend_requests(text);
