/*
  # Fix Challenge Flow and Add Invitation System

  1. New Columns
    - `last_check_in` - Track when users last checked in
    - `check_in_streak` - Track consecutive check-ins
    - `check_in_notes` - Store user notes for each check-in
  
  2. New Functions
    - `check_in_challenge` - Handle daily progress updates
    - `update_user_stats_on_completion` - Update user stats when challenge completed
    - `get_challenge_history` - Get detailed challenge history with check-ins
    - `send_challenge_invitation` - Send invitations to friends
    - `accept_challenge_invitation` - Accept challenge invitations
    - `get_invitation_details` - Get details about an invitation
*/
