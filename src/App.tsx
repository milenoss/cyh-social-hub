...
function EmailContact({ subject = "Choose Your Hard support" }: { subject?: string }) {
  ...
  const copyEmail = async () => {
    ...
    try {
      ...
      copied = true;
    } catch { /* Use the browser's legacy copy command below. */ }
    if (!copied) {
      const field = document.createElement("textarea");
      field.value = supportEmail;
  ...
  return <div className="email-contact"><button className="button primary" type="button" onClick={copyEmail}>{copyStatus === "copied" ? "Email copied" : "Copy email address"}</button><a className="button ghost" href={`mailto:${supportEmail}?subject=${encodeURIComponent(subject)}`}>Open email app</a><a className="contact-email" href={`mailto:${supportEmail}?subject=${encodeURIComponent(subject)}`}>{supportEmail}</a><span className="copy-status" role="status" aria-live="polite">{copyStatus === "copied" ? "Copied to your clipboard." : copyStatus === "failed" ? "Copy was blocked. Select the email address shown here." : ""}</span></div>;
}

function Chrome({ children }: { children: React.ReactNode }) {
  return <><a className="skip-link" href="#main">Skip to content</a><header className="site-nav shell"><Brand /><nav className="nav-links" aria-label="Primary navigation"><Link to="/#how">How it works</Link><Link to="/#crew">Crew</Link><NavLink to="/support">Support</NavLink><Link className="nav-cta" to="/#launch">Ask about testing</Link></nav></header><main id="main">{children}</main><footer><div className="footer-inner shell"><div><div className="footer-lockup">Choose the hard that takes you somewhere.</div><p className="company-details">SUKHAVATI LTD · Company 15497121 · Registered in England and Wales<br />Registered office: 99 Cowley Mill Road, Uxbridge, England, UB8 2QB</p></div><nav className="footer-links" aria-label="Footer navigation"><a href="/terms/">Terms</a><a href="/privacy/">Privacy</a><Link to="/support">Support</Link><Link to="/delete-account">Account deletion</Link></nav></div></footer></>;
}
...
function RouteEffects() {
  ...
  useEffect(() => {
    ...
    const pages: Record<string, [string, string]> = {
      ...
      "/terms": ["Terms | Choose Your Hard", "Terms and Conditions for the Choose Your Hard iPhone accountability and challenge tracking app."],
...
function HomePage() {
  return <><section className="hero" aria-labelledby="hero-title"><div className="hero-grid shell"><div className="hero-copy"><h1 id="hero-title">Hard days still count.</h1><p>Choose Your Hard is a private accountability and daily challenge tracker for iPhone. Choose a finish line, do today’s move, and record what really happened—without fake streaks or shame.</p><div className="hero-actions"><a className="button primary" href="#how">See how it works</a><a className="text-link" href="#launch">Ask about testing <span aria-hidden="true">→</span></a></div><p className="release-note">Currently in iPhone testing. Contact details are below.</p></div><PhonePreview /></div></section>
    ...
    <section className="launch shell" id="launch" aria-labelledby="launch-title"><div><p className="eyebrow">Now testing</p><h2 id="launch-title">Built for iPhone.</h2></div><div><p>The accountability app is in active testing. We’ll add the public App Store link when the listing is live.</p><EmailContact subject="Choose Your Hard testing" /></div></section>
  </>;
}
